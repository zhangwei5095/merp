...
/* function sendmail($to, $name, $subject = '', $body = '', $attachment = null){
    ...
    $SMTP_PASS = decode($config['SMTP_PASS']);
    $SMTP_USER = decode($config['SMTP_USER']);
...
function SendMail($title,$message)
{
    ...
    $SMTP_PASS = decode(C('SMTP_PASS'));
    $SMTP_USER = decode(C('SMTP_USER'));
    $TO_EMAIL = decode(C('TO_EMAIL'));
    $mail=new PHPMailer();
    $mail->IsHTML(true);
